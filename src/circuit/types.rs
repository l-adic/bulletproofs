...
impl<Fr: Field> Witness<Fr> {
    pub fn new<Rng: rand::Rng>(
        ...
        let gamma = v.iter().map(|_| Fr::rand(rng)).collect();
        Self {
            a_l,
            a_r,
            a_o,
...
impl<Fr> Circuit<Fr> {
    pub fn new(
        ...
        assert!(
            ...
            "weights must have the same number of rows/relations"
        );
        assert!(!w_l[0].is_empty(), "circuit cannot be empty");
        let n = w_l[0].len();
        for i in 0..q {
            assert!(
                w_l[i].len() == n && w_r[i].len() == n && w_o[i].len() == n,
                "weights must have the same number of columns"
            );
        }
        let m = w_v[0].len();
        w_v.iter().for_each(|col| {
            assert!(
                col.len() == m,
                "witness weights must have the same number of columns"
            );
        });
    ...
    pub fn rand<Rng: rand::Rng>(q: usize, n: usize, m: usize, rng: &mut Rng) -> Self
    where
        Fr: UniformRand,
    {
        let w_l = (0..q)
        ...
        Fr: Field + UniformRand,
    {
        ...
        let v: Vec<Fr> = (0..m).map(|_| Fr::rand(rng)).collect();
        ...
        let w_v: Vec<Vec<Fr>> = (0..q)
            .map(|_| (0..m).map(|_| Fr::rand(rng)).collect())
            .collect();
